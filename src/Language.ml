  ...
  struct
    ...
    *)                                                       
    ...
    let rec eval env ((st, i, o, r) as conf) = function
            ...
            | Array arr ->
                            let (st, i, o, rarr) = eval_list env conf arr in
                            (st, i, o, Some (Value.of_array rarr))
            | String str ->
            ...
            | Elem (arr, idx) ->
                            let (_, _, _, Some rarr) as conf' = eval env conf arr in
                            let (st, i, o, Some ridx) = eval env conf' idx in
                            Builtin.eval (st, i, o, None) [rarr; ridx] "$elem"
            | Length t ->
                            let (st, i, o, Some rt) = eval env conf t in
                            Builtin.eval (st, i, o, None) [rt] "$length"
            | Call (name, args) ->
    ...
    *)
    ostap (   
      ...
      parseArray:
              "[" elems:parseArgElems "]" {Array elems}
            | x:IDENT                     {Var x};
      ...
      parseElem:
               arr:parseArray " "* idxs:!(Util.listBy)[ostap (" "*)][parseIdx]
               {List.fold_left (fun x y -> Elem (x, y)) arr idxs};
          
      primary:
        name:IDENT "(" args:parseArgElems ")" {Call (name, args)}
      | n:DECIMAL                             {Const n}
      | -"(" parse -")"
      | x:(parseElem|parseArray) "." "length" {Length x}
      | parseElem
      | parseArray
      | x:IDENT   {Var x}
      | "\'" ch:IDENT "\'"                    {Const (Char.code (String.get ch 0))}
      | "\"" str:IDENT "\""                   {Array (List.map (fun x -> Const (Char.code x))
  ...
  struct
    ...
    *)
    ...
    let rec eval env ((st, i, o, r) as conf) k stmt =
            ...
            | k, Assign (name, idxs, x) ->
                            let (_, _, _, idxs) = Expr.eval_list env conf idxs in
                            let (st, i, o, Some rx) = Expr.eval env conf x in
                            eval env (update st name rx idxs, i, o, None) Skip k
            | k, Seq (stm1, stm2) ->
    ...
    ostap (
      parse: seq | atom;
        ...
        assign:    x:IDENT idxs:parseIdxs ":="
                   expr:!(Expr.parse)                 {Assign (x, idxs, expr)};
        ...
        parseIdxs:   <x::xs> :!(Util.listBy)
                              [ostap(" "*)][parseIdx] {x::xs}
                   | ""                               {[]};
                   
        parseIdx: "[" idx:!(Expr.parse) "]"           {idx}
    )
